...
- optimistic update: update state + UI as though b/e call succeeded but rollback
  if fails

- FB pattern: pass dispatch of reducer via context but extract dispatch calls so better guaranteed called in right order (also easier to test)

still the case? context provider/consumers in React DevTools just display as
Context.Provider and Context.Consumer. That doesn't do a good job
differentiating itself from other contexts that may be in your app. Luckily, you
can set the context displayName and it'll display that name for the Provider and
Consumer.
