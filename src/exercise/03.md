...
composability of a parent components that renders all children wrapped in context so that items in context available at all levels vs prev exercies where cloned children with added props only worked 1 level deep (even with composable many siblings some could have a simple div wrapper that breaks sibling hierarchy)

```jsx
ToggleContext.displayName
```
